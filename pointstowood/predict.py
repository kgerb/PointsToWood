...
def get_path(location_in_pointstowood: str = "") -> str:
    ...
    if not match:
        raise ValueError(
            '"PointsToWood/pointstowood" not found in the current working directory path'
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Resolution to which point cloud is downsampled [m]",
    )
    parser.add_argument(
        "--grid_size",
        type=float,
        nargs="+",
        default=[2.0, 4.0],
        help="Grid sizes for voxelization",
    )
    parser.add_argument(
        "--min_pts", type=int, default=128, help="Minimum number of points in voxel"
    )
    parser.add_argument(
        "--max_pts", type=int, default=16384, help="Maximum number of points in voxel"
    )
    parser.add_argument(
        "--model", type=str, default="model.pth", help="path to candidate model"
    )
    parser.add_argument(
        "--is-wood",
        default=0.5,
        type=float,
        help="a probability above which points within KNN are classified as wood",
    )
    parser.add_argument(
        "--any-wood",
        default=1,
        type=float,
        help="a probability above which ANY point within KNN is classified as wood",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="cuda:1",
        help="GPU device to use (e.g., cuda:0, cuda:1, cuda:2). Default: cuda:1",
    )
    ...
    if args.gpu.startswith("cuda:"):
        gpu_id = int(args.gpu.split(":")[1])
        if not torch.cuda.is_available():
            print(f"Warning: CUDA is not available. Falling back to CPU.")
            args.gpu = "cpu"
        elif gpu_id >= torch.cuda.device_count():
            print(
                f"Warning: GPU {gpu_id} not available. Available GPUs: 0-{torch.cuda.device_count() - 1}. Using GPU 0."
            )
            args.gpu = "cuda:0"
        else:
        ...
        print(f"Warning: Invalid GPU device '{args.gpu}'. Using CPU.")
        args.gpu = "cpu"
    ...
    for point_cloud_file in args.point_cloud:
        ...
        args.pc, args.headers = load_file(
            filename=point_cloud_file, additional_headers=True, verbose=False
        )
        args.pc, args.headers, args.reflectance = preprocess_point_cloud_data(args.pc)

        print(f"Voxelising to {args.grid_size} grid sizes")
        preprocess(args, device=args.gpu)
