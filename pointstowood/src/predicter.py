...
class TestingDataset(Dataset, ABC):
    ...
    def __getitem__(self, index):
        ...
        pos = torch.as_tensor(point_cloud[:, :3], dtype=torch.float).requires_grad_(
            False
        )
        reflectance = torch.as_tensor(
            point_cloud[:, self.reflectance_index], dtype=torch.float
        )
        ...
        data = Data(
            pos=pos, reflectance=reflectance, local_shift=local_shift, sf=scaling_factor
        )
        return data
        ...
        for i in prange(num_neighborhoods):
            ...
            if any_wood != 1:
                ...
                for j in range(num_classes):
                    class_votes[j] = np.sum(
                        (nbr_classification[i, :, -2] == j)
                        * nbr_classification[i, :, -1]
                    )
                labels[i, 0] = np.argmax(class_votes)
        return labels

    def collect_predictions(self, classification, original):
        # Remove duplicate columns and existing prediction columns more safely
        columns_to_drop = [
            c for c in original.columns if c in ["label", "pwood", "pleaf"]
        ]
        if columns_to_drop:
            original = original.drop(columns=columns_to_drop)

        # Ensure no duplicate column names
        original.columns = original.columns.astype(str)
        ...
        if os.path.exists(indices_file):
            ...
            _, indices = kd_tree.query(
                original.values[:, :3], k=32 if self.any_wood != 1 else 64
            )
        ...
        original.loc[:, "label"] = labels[:, 0]
        original.loc[:, "pwood"] = labels[:, 1]
        return original
...
def SemanticSegmentation(args):
    ...
    try:
        ...
        raise Exception(
            f"No model loaded at {os.path.join(args.wdir, 'model', args.model)}"
        )
    ...
    test_dataset = TestingDataset(
        voxels=args.vxfile, device=device, max_pts=args.max_pts
    )

    batch_sampler = BalancedBatchSampler(test_dataset, args.batch_size)

    test_loader = DataLoader(
        test_dataset, batch_sampler=batch_sampler, num_workers=0, pin_memory=True
    )
    ...
    if not output_list:
        raise ValueError(
            "No voxels were processed. Check if your point cloud meets the minimum requirements (min_pts, max_pts) and is in the correct format."
        )
    ...
    if args.verbose:
        print("Spatially aggregating prediction probabilites and labels...")
