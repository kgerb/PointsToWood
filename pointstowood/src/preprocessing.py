...
class Voxelise:
    ...
    def gpu_ground(self):
        ...
        x_bins = torch.arange(
            x_min, x_max + grid_resolution, grid_resolution, device=self.device
        )
        y_bins = torch.arange(
            y_min, y_max + grid_resolution, grid_resolution, device=self.device
        )
...
def preprocess(args, device="cuda:1"):
    n_z = Voxelise(
        args.pc,
        vxpath=args.vxfile,
