...
class Voxelise:
    ...
    def quantile_normalize_reflectance(self):
        ...
        normalized_reflectance = torch.erfinv(2 * empirical_quantiles - 1) * torch.sqrt(
            torch.tensor(2.0)
        ).to(reflectance_tensor.device)
        min_val = normalized_reflectance.min()
        max_val = normalized_reflectance.max()
        scaled_reflectance = (
            2 * (normalized_reflectance - min_val) / (max_val - min_val) - 1
        )
    ...
    def gpu_ground(self):
        ...
        x, y, z = (
            self.pos[:, 0].contiguous(),
            self.pos[:, 1].contiguous(),
            self.pos[:, 2].contiguous(),
        )
        print(f"Processing {len(self.pos)} points...")

        grid_resolution = 5.0
        x_min, x_max = torch.min(x), torch.max(x)
        y_min, y_max = torch.min(y), torch.max(y)
        print(
            f"Point cloud bounds: X[{x_min:.1f}, {x_max:.1f}], Y[{y_min:.1f}, {y_max:.1f}]"
        )

        x_bins = torch.arange(
            x_min, x_max + grid_resolution, grid_resolution, device="cuda"
        )
        y_bins = torch.arange(
            y_min, y_max + grid_resolution, grid_resolution, device="cuda"
        )
        print(
            f"Created {len(x_bins)} x {len(y_bins)} = {len(x_bins) * len(y_bins)} grid cells"
        )
        ...
        print("Finding unique grid cells...")
        _, grid_inverse_indices = torch.unique(grid_indices, return_inverse=True)

        print("Computing minimum Z values per grid cell...")
        ...
        normalized_z = z - min_z_per_point
    ...
    def reflectance2intensity(self):
        ...
        if max_reflectance == min_reflectance:
            raise ValueError(
                "max_reflectance and min_reflectance are equal, leading to division by zero."
            )
        scaled_reflectance = (reflectance_tensor - min_reflectance) / (
    ...
    def write_voxels(self):
        ...
        if "n_z" not in self.pos.columns:
            ...
            self.pos = torch.tensor(self.pos.values, dtype=torch.float).to(
                device="cuda"
            )
        ...
        if reflectance_not_zero:
            ...
            if weight.sum() == 0:
                raise ValueError(
                    "All weights are invalid. Check the reflectance values."
                )
            weight = weight + 1e-8
            ...
            if voxel_indices.size(0) > self.maxpoints:
                if reflectance_not_zero:
                    voxel_indices = voxel_indices[
                        torch.multinomial(weight[voxel_indices], self.maxpoints)
                    ]
                else:
                    voxel_indices = voxel_indices[
                        torch.randint(0, voxel_indices.size(0), (self.maxpoints,))
                    ]
